...
$conn = new XMPPHP_XMPP('localhost',5275,'admin','1234', 'xmpphp','sofidcom', $printlog=true, $loglevel=XMPPHP_Log::LEVEL_VERBOSE);

try {
$xmppPassword='1234';
$xmppUserName='wilbert2';
$conn->connect();
$conn->registerNewUser($xmppUserName, $xmppPassword); // User name who need to register $xmppUserName, $xmppPassword
$conn->disconnect();
...
class XMPPHP_XMLStream {
	...
	public function connect($timeout = 30, $persistent = false, $sendinit = true) {
		...
		do {
			...
			try {
				$this->socket = stream_socket_client("$conntype://{$this->host}:{$this->port}", $errno, $errstr,100, $conflag);
			} catch (Exception $e) {
		...
		} while (!$this->socket && (time() - $starttime) < $timeout*2);
...
echo "ESTE ES EL REAL PASSWORD: ".$password;
echo '<br>';		
if (!isset($password))
...
$this->send($xml);
echo "YAAAAAAAAAAAAAAAAAAAAAAAAA".$password;
}
...
class XMPPHP_BOSH extends XMPPHP_XMPP {
                ...
                public function connecta($server, $wait='1', $session=true) {
